...
{   
	...
	//код определени€ частоты вращени€ двигател€
...
{ 
  ...
  SIXSTEP_parameters.direction = DIRECTION;
	...
	MC_SixStep_GetCurrentPosition();//ќпредел€ем текущее положение ротора
}
...
void 			MC_SixStep_Handler(void)
{
	...
	{
			case SIXSTEP_STATUS_INIT: //Ќахождение нач. положени€ ротора
			{						
					MC_SixStep_InitMainData();
